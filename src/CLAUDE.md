...
分级配置体系：`VISION_API_BASE_URL` / `VISION_API_KEY` / `VISION_MODEL_NAME` 作为默认值，每工具可独立覆盖任意字段（`VISION_{DESCRIBE|LOCATE|OCR|VIDEO}_{BASE_URL|API_KEY|MODEL}`）。全部 OpenAI 兼容接口。
